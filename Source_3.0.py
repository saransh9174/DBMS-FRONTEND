...
try:
    cur.execute("use syga")  #DB name
except:
    cur.execute("create database syga")
    cur.execute("use syga")
try:
    cur.execute("describe registration") #table
except:

    cur.execute("create table registration(id int,empname varchar(30),mobile varchar(30),salary int)") #creating table
def GetValue(event):
    e1.delete(0, END)
    e2.delete(0, END)
    e3.delete(0, END)
...
def Add():
    ...
    mysqldb=mysql.connector.connect(host="localhost",user="root",password="admin",database="syga")
